...
Function IniciarSAP(usuario As String, contrasena As String) As Boolean
    On Error Resume Next ' Manejar errores de conexión
    ...
    If credenciales(0) = "" Or credenciales(1) = "" Then
        ...
        'UserForm.Show
        ' El código se detendrá hasta que se cierre el UserForm
        ' Luego, las credenciales se actualizarán desde el UserForm
        User = usuario
        Pass = contrasena
        Unload Inicio
    Else
    ...
    If connection.Children.Count > 0 Then
        ...
        If session.findById("wnd[0]/usr/txtRSYST-BNAME", False) Is Nothing Then
            ' No estamos en la pantalla de login, la sesión ya está activa
            'MsgBox "Sesión ya iniciada, se omite el logueo."
        Else
