    ...
    If Not sapGuiAuto Is Nothing Then
        ...
        Do While InStr(1, session.findById("wnd[0]").Text, "SAP Easy Access") = 0
            ...
            If Err.Number <> 0 Then
                ' Si hay un error, intenta presionar el botón "Cancelar" en la ventana emergente
                On Error Resume Next
                session.findById("wnd[1]/tbar[0]/btn[12]").press ' Presiona el botón "Cancelar"
                On Error GoTo 0
                ' Restablecer el número de error
                Err.Clear
            End If
            On Error GoTo 0
